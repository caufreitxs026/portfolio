...
import CompetenceSection from '@/components/CompetenceSection'; // NOVO COMPONENTE
import { ProjectSkeleton, ExperienceSkeleton } from '@/components/Skeletons';
import { usePortfolioData } from '@/hooks/usePortfolioData';

export default function Home() {
  // Agora buscamos skills e certificados também
  ...
  return (
    <main className="min-h-screen text-slate-300 selection:bg-emerald-500 selection:text-white overflow-x-hidden">
      ...
      <AnimatePresence>
        {!isGameOpen && (
          <motion.button
            ...
            className={`
              fixed bottom-24 right-8 p-3 w-12 h-12 flex items-center justify-center rounded-full shadow-2xl z-[90] transition-colors duration-300 border backdrop-blur-md group
              ${isSecretMode 
      ...
      {/* --- NOVA SEÇÃO: COMPETÊNCIAS E CERTIFICADOS --- */}
      {/* Passamos o isSecretMode para ela se adaptar ao tema */}
      <CompetenceSection 
        skills={skills} 
