...
export default function BackgroundWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div 
      ...
      `}
    >
      {/* --- GALAXIA ESCURA (AURORA) --- */}
      <div 
        className={`fixed inset-0 z-0 transition-opacity duration-1000 pointer-events-none ${theme === 'dark' ? 'opacity-100' : 'opacity-0'}`}
      >
      ...
      {/* --- GALAXIA CLARA (NEBULA) --- */}
      <div 
        className={`fixed inset-0 z-0 transition-opacity duration-1000 pointer-events-none ${theme === 'light' ? 'opacity-100' : 'opacity-0'}`}
      >
         {/* Luz suave superior */}
         ...
         {/* Padrao de Grid Sutil para dar textura tecnica */}
