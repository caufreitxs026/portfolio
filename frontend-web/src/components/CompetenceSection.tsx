...
const NeuralScene = dynamic(() => import('./NeuralNexus/Scene'), { 
    ...
    loading: () => (
        <div className="w-full h-full flex items-center justify-center bg-slate-900/20 rounded-2xl border border-slate-800">
             <span className="text-emerald-500 font-mono text-xs animate-pulse">LOADING...</span>
        </div>
    )
});
            ...
            <div className="grid lg:grid-cols-3 gap-8 h-auto">
                ...
                <div className={`lg:col-span-2 h-[600px] lg:h-[700px] w-full relative rounded-2xl overflow-hidden border backdrop-blur-sm group ${containerClass}`}>
                    ...
                    <div className="absolute bottom-4 right-6 pointer-events-none opacity-50 group-hover:opacity-100 transition-opacity duration-500 z-10">
                        <p className={`text-[10px] font-mono uppercase px-2 py-1 rounded border ${theme === 'dark' ? 'bg-slate-900/80 border-white/5 text-slate-500' : 'bg-white/80 border-slate-200 text-slate-600'}`}>
